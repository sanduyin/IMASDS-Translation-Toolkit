...
*   **Full DSi/TWL Enhancement Rebuild**: Complete cryptographic reconstruction of the DSi-enhanced region — sector/block digest tables, seven HMAC groups, ARM9i/ARM7i extraction, and modcrypt — so the localized ROM boots correctly on both NDS and DSi hardware (e.g. via Pico Loader).
...
**All source code in this repository is generated by AI.** The implementation, debugging, and iteration were carried out through collaboration with AI coding assistants. While we strive for correctness, users should review the code carefully before any critical use.
...
*   **完整 DSi/TWL 增强区重建**：对 DSi 增强区域进行完整密码学重建——sector/block digest 摘要表、七组 HMAC、ARM9i/ARM7i 提取与 modcrypt 加密——确保汉化 ROM 在 NDS 与 DSi 实机（如 Pico Loader）上均可正常启动。
